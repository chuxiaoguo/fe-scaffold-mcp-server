 ...
 * 创建脚手架处理函数
 */
export async function handleCreateScaffold(params: CreateScaffoldParams): Promise<string> {
  try {
    // 构建选项
    ...
    // 生成项目统计信息
    const stats = ProjectGenerator.generateProjectStats(result.files);
    ...
    return `❌ 创建脚手架失败: ${error instanceof Error ? error.message : String(error)}`;
  }
}
