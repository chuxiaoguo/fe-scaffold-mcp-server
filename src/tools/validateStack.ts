 ...
 * 验证技术栈处理函数
 */
export async function handleValidateStack(params: ValidateStackParams & {
  language?: string;
  ...
  try {
    ...
    if (hasChanges) {
      ...
      if (options.uiLibrary !== fixedOptions.uiLibrary) {
        response += `- UI组件库: ${options.uiLibrary || '未指定'} → ${fixedOptions.uiLibrary}\n`;
      }
    }
    ...
    return `❌ 验证技术栈失败: ${error instanceof Error ? error.message : String(error)}`;
  }
}
...
function buildOptionsFromParams(params: any): ScaffoldOptions {
  ...
  return {
    ...
    testing: {
      framework: params.testingFramework || (params.buildTool === 'vite' ? 'vitest' : 'jest'),
      mockSolution: params.mockSolution || (params.buildTool === 'vite' ? 'msw' : 'webpack-proxy'),
    },
    bundleAnalyzer: params.buildTool === 'vite' ? 'rollup-plugin-visualizer' : 'webpack-bundle-analyzer',
  };
}
...
function getBestPractices(options: ScaffoldOptions): string {
  ...
  if (missingQualityTools.length > 0) {
    practices.push(`建议启用以下代码质量工具: ${missingQualityTools.join(', ')}`);
  }
  
  // Mock方案推荐
  if (options.testing.mockSolution === 'webpack-proxy' && options.buildTool === 'vite') {
 ...
 * 计算兼容性评分
 */
function calculateCompatibilityScore(validation: ReturnType<typeof StackValidator.validate>): number {
  let score = 100;
